...
use constant MAP_SUBITEM_FILE => {
  ...
  '1009' => ["BODY_RTF",	0], # RTF Compressed version of body
...
sub mime_object {
  ...
  if ($self->_IsMultiPart) {
    ...
    if ($self->{BODY_RTF}) {
		...
		# temp hack
		open TMP, ">.tmp-$$";
		print TMP $self->{BODY_RTF};
		close TMP;
		my $rtf = `~/thingy/minipdf/new/libstream/test/msg/rtfdecompr .tmp-$$ | tail +2`;
		unlink ".tmp-$$";

      $self->_SaveAttachment($bodymime, {
	MIMETYPE => 'text/rtf',
	ENCODING => '8bit',
	DATA => $rtf, #$self->{BODY_RTF},
	DISPOSITION => 'inline',
      });
    }
...
sub _AttachmentItem {
  ...
  if ($PPS->{Type} == DIR_TYPE) {
    ...
    if ($property eq '3701') {	# Nested MSG file
      ...
      $msgp->parse($PPS);
	  if (not defined($msgp->mime_object)) {
	  	warn "Failed to parse nested MSG file."
	  } else {
...
sub _SaveAttachment {
  ...
  if ($handle = $ent->open("w")) {
  	# debug
  	if (not defined($att->{DATA})) {
		warn "Attachmed with undefined data.\n";
	} else {
...
sub _AddHeaderField {
  my ($self, $mime, $fieldname, $value) = @_;
	...
	# for some strange reason, i got tot here with $mime being undefined.
	# debug that, but for now:
	return unless defined($mime);
...
sub _ExpandAddressList {
  my ($self, $names) = @_;
	...
	# this stuffed up. $names undefined. debug
